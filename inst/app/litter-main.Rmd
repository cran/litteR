...
d_grp <- d_type %>%
  ...
  map_df(function(x) {
    d_type %>%
      ...
      group_by(spatial_code, date) %>%
      summarise(count = sum(count)) %>%
      ungroup %>%
      filter(!are_na(count)) %>%
      mutate(type_name = x)}) 

# computing relative group totals (relative w.r.t. TC)
d_grp <- d_grp %>% 
  filter(type_name == "TC") %>%
  select(spatial_code, date, TC = count) %>%
  right_join(d_grp, by = c("spatial_code", "date")) %>%
...
d_ltr <- d_grp %>%
  filter(type_name == "TC") %>%
  select(spatial_code, date, TC = count) %>%
  right_join(d_ltr, by = c("spatial_code", "date")) %>%
  mutate(rel_count = 100 * count / TC) %>%
  select(-TC) %>%
